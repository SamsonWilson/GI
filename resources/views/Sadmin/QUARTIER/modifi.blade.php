                    ...
                    <div class="card border-top border-0 border-4 border-white">
                        <div class="card-body p-5">
                            ...
                            <form class="row g-3" method="POST"
                                action="{{ url('update_quartier/' . $quartier->id) }}",enctype="multipart/form-data">
                                @csrf
                                @method('PUT')
                                <div class="col-md-12">
                                    <label for="inputLastName" class="form-label">Nom du Quartier </label>
                                    <input type="text" class="form-control" value="{{ $quartier->nom }}" name="nom"
                                        id="inputLastName" placeholder="Nom de la ville">
                                </div>
                                <div class="col-12">
                                    <label for="inputAddress" class="form-label">Description de la ville</label>
                                    <textarea class="form-control" value="{{ $quartier->description }}" name="description" id="inputAddress"
